   ...
   <div class="panel panel-flat">
      ...
      <div class="row" style="font-family: sans-serif; padding: 10px;">
        @foreach ($group as $value)
           <div class="col-md-6">
          <p style="font-size: 16px;">{{$value->group->name}}: </p>
          @foreach ($code_is_used as $element)
          @if ($value->id_group==$element->id_group)
            <p><strong>Đã sử dụng <span style="color: red;">{{$element->number_is_used}}</span>/{{$value->number_ingroup}}</strong></p>
          @endif
          @endforeach
        </div>
        @endforeach
