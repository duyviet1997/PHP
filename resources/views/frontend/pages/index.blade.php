                            ...
                            <div class="single-product" class="uk-width-medium-1-2 uk-flex" data-uk-scrollspy="{cls:'uk-animation-scale-up', delay: 400}">
                                <div class="product-f-image">
                                    ...
                                    <div class="product-hover">
                                        ...
                                        {{-- <a id="check-inventory" data-id="{{$value->id_product}}" class="check-inventory"><i class="fa fa-link"></i></a> --}}
                                    </div>
                                </div>
                                ...
                                {{-- <div class="text-center">
                                    @if (empty($value->ProductStore->number) )
                                        <span>Tạm hết hàng</span>
                                    @else  <span>{{$value->ProductStore->number}}</span>
                                    @endif
                        ...
                        @endif
                            ...
                            <div class="product-wid-price">
                                @if ($value->product->promotion_price ==0)
                                    <ins>{{number_format($value->product->price,0,',','.')}}</ins>
                                @else
                                <ins>{{number_format($value->product->promotion_price,0,',','.')}}</ins> <del>{{$value->product->price}}</del>
                                @endif
                            </div>                            
                        </div>
                        @endforeach
                        ...
                        @foreach ($discount_product as $value)
                          <div class="single-wid-product">
                            ...
                            <div class="product-wid-price">
                                ...
                                <ins>{{number_format($value->promotion_price,0,',','.')}}</ins> <del>{{number_format($value->price,0,',','.')}}</del>
                                @endif
                            </div>                            
                        </div>
                        @endforeach
