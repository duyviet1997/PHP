...
{
      public function __construct(ProductsRepository $ProductsRepo,BillDetailRepository $BillDetailRepo, BillRepository $BillRepo,CustomerRepository $CustomerRepo)
    {
...
//         $users = User::whereMonth('created_at', date('m'))->get();
    ...
    public function reportInDate(Request $request)
    {
      ...
      if ($request->get('end_date')!=NULL) {
        ...
        return view('backend.report.sales.reportSales',compact('bills','total','count','chart','start_date','end_date','sub_orderd','sub_accept','sub_finish','sub_cancel','orderd','accept','finish','cancel'));
    }
     ...
     public function inventory(Request $request)
    {
       ...
       $products = $this->ProductsRepo->getAllProductsInStore($request);
        ...
        foreach ($store as $value) {
          $count_store.=$value->name . ',';
        }
        $count_store=explode(',',trim($count_store,','));
        $pro_store = ProductStore::all();
        $inventory=ProductStore::select( 'id_store', DB::raw( 'SUM(number) as total_number' ) )
                        ->groupBy('id_store')->orderBy('id_store','ASC')->get();
       
        $count_inventory='';
        foreach ($inventory as $value) {
          $count_inventory.=$value->total_number . ',';
        }
        $count_inventory=explode(',',trim($count_inventory,','));
        $id_store=1;
        if(!empty($request->get('id_store')))
        {
             $id= $request->get('id_store');
              $id_store= Store::find( $id)->first();
               $id_store=  $id_store->id;
        }
        $chart = new SampleChart;
        $chart->labels($count_store);
        $chart->dataset('Số lượng tồn kho', 'bar', $count_inventory)->backgroundColor('#0000FF');
        $chart->title('Biểu đồ tồn kho');
        $chart->height(400);
        $chart->width(200);

         return view('backend/report/store/inventory', compact('products','store','id_store','pro_store','chart'));
    }
}
