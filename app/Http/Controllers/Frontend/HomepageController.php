...
class HomepageController extends Controller
{
    ...
    public function addToWishList(Request $request)
    {
        ...
        $img=explode('/', $request->get('img'));
        $input['image']='uploads/'.$img[count($img)-1];
   ...
   public function get_inventory(Request $request)
   {
    ...
    $check=ProductStore::select( 'id_product', DB::raw( 'SUM(number) as total' ) )
                      ->where('id_product',$id_product)
                      ->where('number','>',0)->groupBy('id_product')->first();
